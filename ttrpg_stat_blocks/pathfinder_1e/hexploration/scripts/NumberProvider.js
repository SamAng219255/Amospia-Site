...
export default class NumberProvider extends Serializable {
	...
	async execute(chainData) {
		return this.#operationFunc(chainData);
	}
	...
	static async execute(operation, chainData) {
		if(typeof operation == "number")
			return operation;
		if(operation == null)
			return operation;
		return NumberProvider.OPERATIONS[operation.type].func(operation)(chainData);
	}
	...
	static get OPERATIONS() {
		if(!this.#OPERATIONS)
			this.#OPERATIONS = {
				"constant": {
					func: ({value}) => async (chainData) => value,
					args: new ArgConditions({id: "value", can_accept: ["number"]}),
					label: "Constant",
					desc: "Returns the value.",
				},
				"sum": {
					func: ({values}) => async (chainData) => (await resolveProviders(chainData, values)).reduce((sum, summand) => sum + summand, 0),
					args: new ArgConditions({id: "values", multi: true}),
					label: "Add",
					desc: "Returns the total sum of all of the values.",
				},
				"sub": {
					func: ({minuend, subtrahend}) => async (chainData) => (([minuend, subtrahend]) => minuend - subtrahend)(await resolveProviders(chainData, minuend, subtrahend)),
					args: new ArgConditions("minuend", "subtrahend"),
					label: "Subtract",
					desc: "Returns the minuend minus the subtrahend.",
				},
				"mul": {
					func: ({values}) => async (chainData) => (await resolveProviders(chainData, values)).reduce((product, factor) => product * factor, 1),
					args: new ArgConditions({id: "values", multi: true}),
					label: "Multiply",
					desc: "Returns the total product of all of the values.",
				},
				"div": {
					func: ({dividend, divisor}) => async (chainData) => (([dividend, divisor]) => dividend / divisor)(await resolveProviders(chainData, dividend, divisor)),
					args: new ArgConditions("dividend", "divisor"),
					label: "Divide",
					desc: "Returns the dividend divided by the divisor.",
				},
				"sqr": {
					func: ({base}) => async (chainData) => (x => x * x)(await NumberProvider.execute(base, chainData)),
					args: new ArgConditions("base"),
					label: "Square",
					desc: "Returns the square of the base. Aka, the base times itself.",
				},
				"cub": {
					func: ({base}) => async (chainData) => (x => x * x * x)(await NumberProvider.execute(base, chainData)),
					args: new ArgConditions("base"),
					label: "Cube",
					desc: "Returns the cube of the base. Aka, the base to the 3rd power.",
				},
				"pow": {
					func: ({base = 2, exponent}) => async (chainData) => (([base, exponent]) => Math.pow(base, exponent))(await resolveProviders(chainData, base, exponent)),
					args: new ArgConditions({id: "base", def: () => 2}, "exponent"),
					label: "Power",
					desc: "Returns the base to the power of the exponent.",
				},
				"clamp": {
					func: ({value, min = 0, max = 1}) => async (chainData) => (([value, min, max]) => value < min ? min : value > max ? max : value)(await resolveProviders(chainData, value, min, max)),
				...
				"smooth_step": {
					func: ({coordinate, min = 0, max = 1, slope = 3}) => async (chainData) => (([t, min, max, slope]) => min + (max - min) / (1 + Math.exp(-4 * slope * (t - 0.5))))(await resolveProviders(chainData, coordinate, min, max, slope)),
				...
				"rand": {
					func: ({min = 0, max = 1}) => async (chainData) => (([min, max]) => min + (Math.random() * (max - min)))(await resolveProviders(chainData, min, max)),
					args: new ArgConditions({id: "min", def: () => 0}, {id: "max", def: () => 1}),
					label: "Random",
					desc: "Returns a random number between the minimum and maximum.",
				},
				"randInt": {
					func: ({min = 0, max}) => async (chainData) => (([min, max]) => min + Math.floor(Math.random() * (1 + max - min)))(await resolveProviders(chainData, min, max)),
					args: new ArgConditions({id: "min", def: () => 0}, "max"),
					label: "Random Integer",
					desc: "Returns a random integer between the minimum and maximum.",
				},
				"die": {
					func: ({sides, count = 1}) => async (chainData) => (([sides, count]) => {
						let total = 0;
						for(let i = 0; i < count; i++) 
							total += Math.floor(Math.random() * sides) + 1;
						return total;
					})(await resolveProviders(chainData, sides, count)),
					args: new ArgConditions({id: "count", def: () => 1}, {id: "sides", def: () => 6}),
					label: "Dice",
					desc: "Returns a random number like rolling a number of dice.",
				},
				"triangle": {
					func: ({component1, component2 = null}) => async (chainData) => (([a, b]) => (Math.random() * a) + (Math.random() * (b ?? a)))(await resolveProviders(chainData, component1, component2)),
				...
				"triangleInt": {
					func: ({component1, component2 = null}) => async (chainData) => (([a, b]) => Math.floor(Math.random() * a + 1) + Math.floor(Math.random() * (b ?? a) + 1))(await resolveProviders(chainData, component1, component2)),
					args: new ArgConditions("component1", {id: "component2", optional: true}),
				...
				"weighted": {
					func: ({options}) => async (chainData) => {
						const weights = await resolveProviders(chainData, options.map(option => option.weight));
						...
						for(let i = 0; i < options.length; i++) {
							...
							if(randIndex < growingCeiling)
								return NumberProvider.execute(options[i].value, chainData);
						}

						return NumberProvider.execute(options[options.length - 1].value, chainData);
					},
				...
				"if": {
					func: ({query = FeatureQuery.TRUE, valueIfTrue = 1, valueIfFalse = 0}) => async (chainData) => (await FeatureQuery.execute(query, chainData)) ? NumberProvider.execute(valueIfTrue, chainData) : NumberProvider.execute(valueIfFalse, chainData),
				...
				"lerp": {
					func: ({coordinate, start, end}) => async (chainData) => (([t, arg1, arg2]) => t < 0 ? arg1 : t > 1 ? arg2 : (1 - t) * arg1 + t * arg2)(await resolveProviders(chainData, coordinate, start, end)),
				...
				"ifs": {
					func: ({cases, def}) => async (chainData) => {
						for(const {condition, valueIfTrue} of cases) {
							const pass = await FeatureQuery.execute(condition, chainData);
							if(pass)
								return await NumberProvider.execute(valueIfTrue, chainData);
						}
						return await NumberProvider.execute(def);
					},
					args: new ArgConditions({id: "cases", multi: true, can_accept: {condition: ["query"], valueIfTrue: ["number", "provider"]}}, {id: "def", label: "Default"}),
					label: "Ifs",
					desc: "Checks a number of query and returns the value associated with the first matching query. If all the queries fail, returns the default.",
				}
			};
...
async function resolveSplineNode(chainData, node) {
	const [value, derivative] = await resolveProviders(chainData, node.value, node.derivative);
	return {location: node.location, value, derivative};
}

async function resolveProviders(chainData, ...providers) {
	return (await Promise.all([...providers].map(provider => Array.isArray(provider) ? resolveProviders(chainData, ...provider) : NumberProvider.execute(provider, chainData))));
}