	...
	monsterBlockAuto(
		...
		[
			...
			[
				...
				'type' => 'Su',
				'desc' => 'When is enderman is targetted by an effect such as a spell, attack, or combat maneuver, or when it is in the area of such an effect, an enderman can make a DC 15 reflex save. If the enderman succeeds at this save they momentarily travel to the astral plane before returning to a location within 60 feet. The enderman can return to the same place they left from but they may be subject to any ongoing effect placed on the area. Enderman receive a +10 bonus to this check if the effect requires a projectile, such as an attack with a bow or a ii/fireball/ii spell, other than a bullet from a firearm. The enderman receives the same bonus if the triggering effect required longer than one standard action to activate. As the enderman is not located on the same plane, spells and effects that automatically hit such as ii/magic missile/ii can still be negated.'
			],
			[
				'name' => 'Translocate',
				'type' => 'Su',
				'desc' => 'Enderman can pass through the astral plane to a distant point, doing so, however, leaves the enderman momentarily vulnerable. An enderman provokes an attack of opportunity from all nearby creatures when they arrive. An enderman can teleport up to 60 feet as a move action using this ability. Given more time an enderman can teleport longer distances. As a standard action an enderman can teleport 480 feet and as a full round action an enderman can teleport up to 1,000 miles. Multiple enderman working together can transport themselves furhter distances. When at least two enderman are in contact and use this ability together, they can teleport to any location of the same plane or they travel to another dimension using the same rules and limitations as ii/plane shift/ii. When an enderman teleports using this ability they first create a divinatory sensor at the location to confirm if the site is safe. An enderman does not risk teleporting into a solid object or to an otherwise dangerous location. If the enderman chooses to, they can teleport to a dangerous location but not intersecting a solid object.'
			],
			[
