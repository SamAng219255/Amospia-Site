...
["charcoal",	true,	false],
["iron",		true,	false],
["leatherArmor",true,	false],
["ironArmor",	true,	false]
];
creatureList=[
...
function generateValidInputs(list) {
	eval(
		"for(var i = 0; i<"+list+".length; i++) { "+
			"validInputs=addToArray(validInputs,"+list+"[i][0]) "+
		"} "
	);
}
...
$("body")[0].style="background-image:url('img/backgrounds/"+terrainList[parseInt(terrainList.length*Math.random())][0]+".jpg')";
function fill() {
...
function receive(action,first) {
	if(first==true || totalIn.split(" ")[segment]==undefined) {
		...
		waitTime=0;
		function stuff() {
			if(document.getElementById("inputBox").value=="hunt fish" && inputTest==true) {
				clearInterval(repeater);
				inputTest=false;
				document.getElementById("inputBox").value="";
				transmit("Nein!","computer");
				document.getElementById("forText").innerHTML+="<div style=\"position:relative;height:0;padding-bottom:56.22%\"><iframe src=\"https://www.youtube.com/embed/dQw4w9WgXcQ?autoplay=1&controls=0&loop=1\" style=\"position:absolute;width:100%;height:100%;left:0\" width=\"640\" height=\"360\" frameborder=\"0\" allow=\"autoplay; encrypted-media\" allowfullscreen></iframe></div>";
				receive("tree1",true);
			}
			else if(inputTest==true) {
				clearInterval(repeater);
				inputTest=false;
...
function baseText() {
	...
	for(var i=0; i<eval("map.x"+player.position.x+".y"+player.position.y+".z"+player.position.z+".structures").length; i++) {
		stuffThings=addToArray(stuffThings,[eval("map.x"+player.position.x+".y"+player.position.y+".z"+player.position.z+".structures[i].Name"),1]);
	}
	for(var i=0; i<eval("map.x"+player.position.x+".y"+player.position.y+".z"+player.position.z+".creatures").length; i++) {
		if(eval("map.x"+player.position.x+".y"+player.position.y+".z"+player.position.z+".creatures")[i].count>0) {
			stuffThings=addToArray(stuffThings,[eval("map.x"+player.position.x+".y"+player.position.y+".z"+player.position.z+".creatures[i].Name"),1]);
		}
	}
	for(var i=0; i<here().structures.length; i++) {
		if(here().resources[i].Name=="meteor") {
			stuffThings=addToArray(stuffThings,["meteorite",1]);
		}
	}
...
function meteoroid() {
	...
	if(findIn("array",false,map["x"+astroX].y0["z"+astroZ].resources,"meteor",true)==-1) {
		map["x"+astroX].y0["z"+astroZ].resources.push(["meteor","none",1]);
	}
	else {
		map["x"+astroX].y0["z"+astroZ].resources[findIn("array",0,map["x"+astroX].y0["z"+astroZ].resources,"meteor",true)][2]++;
	}
...
function gameEvent(thisEvent) {
	if(Math.random()<gameEvents[thisEvent].Chance) {
		eval(gameEvents[thisEvent].eventFunction);
	}
}