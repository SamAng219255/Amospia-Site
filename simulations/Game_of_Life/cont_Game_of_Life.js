...
function activate(event) {
	x=Math.floor((event.x-(canvas.offsetLeft-document.getElementById("outerMapBox").scrollLeft))/UCAsize);
	y=Math.floor((event.y-(canvas.offsetTop-window.scrollY))/UCAsize);
	if(foo[x][y]) {
		foo[x][y]=false;
	}
	else {
...
var foo;
function lifer() {foo=round(foo); showLife(foo); console.log("lived"); fooCount++; document.getElementById("fooCounterDiv").innerHTML="Rounds: "+fooCount;}
function emptyProto() {
	var outputArray=[];
	for(var i=0; i<Math.floor(1024/UCAsize); i++) {
		outputArray=addToArray(outputArray,[]);
		for(var j=0; j<Math.floor(1024/UCAsize); j++) {
